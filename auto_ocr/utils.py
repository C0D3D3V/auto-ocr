...
def check_debug() -> bool:
    ...
    return "pydevd" in sys.modules or (
...
def append_list_to_json(json_file_path: str, list_to_append: List[Dict]):
    ...
    json_bytes = orjson.dumps(
        list_to_append, option=orjson.OPT_INDENT_2 | orjson.OPT_APPEND_NEWLINE
    )
    try:
        ...
        logging.error(
            "Error: Could not append List to json: %r Reason: %s", json_file_path, err
        )
        exit(-1)
    finally:
...
class PathTools:
    ...
    def sanitize_path(path: str):
        ...
        norm_path = os.path.normpath(PathTools.remove_start(path, drive_or_unc)).split(
            os.path.sep
        )
        if drive_or_unc:
            norm_path.pop(0)
        ...
        sanitized_path = [
            (
                path_part
                if path_part in [".", ".."]
                else PathTools.to_valid_name(path_part)
            )
